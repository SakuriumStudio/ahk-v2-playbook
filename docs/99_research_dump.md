...
- ポイント: 上記ではFileReadでエラーが発生するとcatchブロックに入り、ExceptionオブジェクトeからMessageプロパティを取得しています。elseブロックは例外が発生しなかった場合に実行され、finallyブロックは成功・失敗を問わず最後に必ず実行されます（リソース解放などに利用）[21]。
...
- ポイント: +AlwaysOnTopでこのGUIウィンドウは常に最前面表示されます[22]。-Captionによりタイトルバーと枠を外し[23]、+ToolWindowでタスクバーに表示しないツールウィンドウになります[24]。NoActivateオプションを指定しているため、GUI表示しても他アプリのフォーカスを邪魔しません。【クリック透過】を実現したい場合、さらに拡張スタイルWS_EX_TRANSPARENT（0x20）を付与します（例：gui.Opt("+E0x20") ※このスタイルを付けるとGUIがマウスクリックを無視し、背後のウィンドウにクリックが届きます）。
...
- ポイント: 初回実行時はINIファイルが存在しなくてもIniWriteで自動作成されます。IniReadでは第4引数にデフォルト値を指定でき、キー/値が見つからない場合にその値が返されます[10]。上記例では初回実行時、設定が無ければ "Guest" と表示され、2回目以降は保存されたユーザー名を表示します。例外処理が必要なケース（例えばファイル書込み失敗など）ではtry-catchを組み合わせてください。
...
- ポイント: WinActive("A")は現在フォアグラウンドにあるウィンドウの存在チェックで、アクティブならそのウィンドウID（ハンドル）を返します[25]。取得できたhwndに対し、WinGetTitleやWinGetProcessNameで詳細情報を取得可能です。上記ホットキーを使えば、現在操作中のウィンドウのタイトルと実行プロセス名をポップアップ表示できます。特定のウィンドウがアクティブか確認するには、例えばif WinActive("Untitled - Notepad")のようにタイトルを渡してbool判定もできます。
...
- ポイント: 冒頭に#SingleInstance Forceを置くことで、同じスクリプトの二重起動を防止しています（既存プロセスがいればそれを終了して新しいプロセスを実行）[19]。また、OnExit(*) => 関数によりスクリプト終了時の処理を登録し、ログに終了タイムスタンプを書き込んでいます。実行中のイベント記録にはOutputDebug()関数でデバッガ表示（DebugViewツール等で監視）したり、FileAppendでログファイルに追記する方法が使えます。上記では開始と終了をそれぞれログに残し、MsgBoxで動作確認できるようにしています。
